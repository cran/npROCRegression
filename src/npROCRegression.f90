...
!	******************************************************************
!	******************************************************************
	subroutine cROC(m0,m1,v0,v1,Err0, Err1,w0,w1,n0,n1,t,nt, &
		SH, ROC)
	implicit none
	integer n0, n1, nt, it
	double precision m0, m1, v0, v1, w0(n0), w1(n1), Err0(n0), &
		Err1(n1), auxM, auxV, auxV1, SH(nt), t(nt), ROC(nt)
...
!	******************************************************************
	...
	integer n,i,j,kbin,p,ifcv,icont,ih,ih0
	double precision x(n),y(n),W(n),Xb(kbin),Yb(kbin),Wb(kbin), &
		d1,xmin,xmax,Area(2),dis1,dis2,Pb(kbin),h, &
	...
	elseif (h.lt.0) then ! smooth effect by CV	
		...
		do ih = 1, nh
			h = haux(ih)					
			call Rfast_(X,Y,n,W,h,p,Xb,Yb,Wb,Pb,kbin,1)
			ErrCV = 0
			do i = 1,kbin
	...
	call Rfast_(X,Y,n,W,h,p,Xb,Yb,Wb,Pb,kbin,0)
1	continue
	deallocate(Pred,X2,W2, B, haux)
	end subroutine
!	******************************************************************
!	******************************************************************
	subroutine Rfast_(X,Y,n,W,h,p,Xb,Yb,Wb,Pb, &
		kbin,ifcv)
	implicit none
	integer n,i,j,kbin,p,ifcv,icont
	double precision, parameter :: deg2rad = 3.14159265/180.0
	double precision x(n),y(n),W(n),Xb(kbin),&
		Yb(kbin), Wb(kbin), Pb(kbin),&
		m, dis1,aux,h,sumw,tanh,h2,min,max,pdf
...
!	******************************************************************
	...
	double precision X(n),alpha(nalpha),Q(nalpha),R,xest
	call qsortd(x,ind,n)
...
!	******************************************************************
	module Mod0
	integer minit,maxit,nit,maxitl,nitl
	double precision eps,Devian,thloc,linc
	integer,allocatable::nf(:)
	double precision,allocatable::Fact(:,:),ErrCV(:)
   	end Module
!	******************************************************************
!	******************************************************************
	Module Modbin
...
!	******************************************************************
	...
	use Data
	implicit none
	deallocate (X,Xp,Mode,Fact,nf,ErrCV,Xpar,Xppar,B)
	end
!	***********************************************************************
!	***********************************************************************
	...
	allocate (X(n,2,npar), Xp(npred,2,npar), mode(2,npar), &
		Fact(n,npar),nf(npar),ErrCV(npar))
	! Type of effects		
	do r=1,npar
		do s=1,2						
			mode(s,r)=0					 
...
!	********************************************************************
!	********************************************************************
	subroutine GAM(n, nvar, npar, Mode0, II, Xdata, Y, W, h2, nbin, p,&
	...
	double precision Xdata(n,nvar-1),Y(n),h2(n,npar),F(n,npar), &
		...
		muhatpred(npred), etak, coeff(20)
	double precision,allocatable::Eta(:),Etap(:)
	integer i,j,s,r, &
		vpar(npar), vnpar(npar), n_vpar, n_vnpar
	double precision devnew,p0,s0,var,Devold,aux,eta0,med,der,xmiss, &
		ifcero
	...
	do nitl = 1,maxitl            
		...
		h0=h2		
		call BackFitInter(n,npar,Z,Wz,Wx,h0,p, &	   ! Input
			vpar,n_vpar,vnpar,n_vnpar,& 			   ! Input
			Eta,F,s0,Etap,Fpred,npred)				   ! Output
...
!	********************************************************************
!	********************************************************************
	subroutine BackFitInter(n,npar,Y2,W2,Wx,h2,p, &
		vpar,n_vpar,vnpar,n_vnpar, &  
		Eta,F,s0, Etap, Fp, np)
	Use Data
	Use Mod0
	...
	integer i,j,n,npar, np, p(npar), n_vpar, n_vnpar, &	 
		vpar(n_vpar), vnpar(n_vnpar)		  
	double precision deltaf,ratio,normf,med,s0,Eta(n),Y2(n),W2(n), &
		F(n,npar),aux, Wx(n), Etap(np), Fp(np,npar)     
	double precision,allocatable::Z2(:),Old(:),Oldp(:),etal(:),&
		etalp(:)
	...
	!	Smooth part
		...
		do j=1,n_vnpar
			...
			mode(1,vnpar(j)),F(1,vnpar(j)), &
			ErrCV(vnpar(j)),vnpar(j), &  
...
!	********************************************************************
!	********************************************************************
	subroutine Finter(X,Y2,n,W,Wx,h2,p,Fact,nf,mode,F, &
		ErrCv,jpar, Xp, Fp, np)
	use modbin
	implicit none
	integer n,nf,gp,family,jpar, np,p
	double precision,allocatable::Vaux(:)
	double precision X(n,2),Y2(n),W(n),Wx(n),mode(2),F(n), &
		Fact(n),ErrCV, Xp(np,2), Fp(np)
	...
	if (mode(1).eq.0.and.mode(2).eq.5) then		! %%%%%%   oo -- Continuous
		call RNP1DFast(X(1,2),Y2,n,W,WX,h2(2),p,kbin,F,Vaux,ErrCv, &
		jpar, Xp (1,2), Fp, np)
	elseif (mode(1).eq.5.and.mode(2).eq.0) then !%%%% Continuous -- ooo
		call RNP1DFast(X(1,1),Y2,n,W,WX,h2(1),p,kbin,F,Vaux,ErrCv, &
		jpar,Xp (1,2), Fp, np)
...
!	*********************************************************
!	*********************************************************
	subroutine RNP1DFast(X,Y,n,WY,WX,h,p,kbin,M0,M1,ErrCV,jpar, &
		Xp,Fp,np)
      implicit none
	integer :: n,kbin,icont,i, p, jpar, np
	double precision :: X(n),Y(n),WY(n),WX(n),M0(n),M1(n),hmin,hmax,s,&
		aux,errmin, ErrCV,xmin,xmax, Xp(np), Fp(np)
...
!	******************************************************************
	...
	double precision X(n),XF(n),Y(n),W(n),Wx(n),Fact(nf),Err,Med,F(n), &  
		Beta0, Xp(np), Fp(np), XFp(np)
	double precision,allocatable::W2(:),Maux(:),Res(:),GrF(:,:), &
		GrFp(:,:)
	double precision h(nf)
	allocate(W2(n),GrF(n,nf),Res(n),Maux(n), GrFp(np, nf))
	...
	do i=1,np
      ...
      deallocate(W2,GrF,Res,Maux, GrFp)
	end
!	*********************************************************
!	*********************************************************
...
!	******************************************************************
	...
	Use Mod0
	implicit none
	integer i,j,l,n,np,p,k,aux, nvpar, vpar(nvpar)
...
!	******************************************************************
	...
	implicit none
	integer nt,i
	double precision t(nt), ROC(nt), ru, res, min
	call random_number(ru)
	min = 1.0
	do i=1,nt
		if(ROC(i).ge.ru) then	
...
!	******************************************************************
	subroutine ROCDirectab(Z0,X0,W0,n0,Z1,X1,W1,n1, &	! Input
		...
		nparm, IIm, nparv, IIv, nparr, IIr, & 			! Partial in Mean, Variance and ROC 	
		npart, IIt, &									! Fit args
		kbin,pm,pv,hm,hv,hroc,iopt, &					! Fit args
		parz, &											! Test		
		Zb,nb,tb,ntb, &									! Prediction	
		pvalue)											! Output
	implicit none
	integer, parameter :: nboot=400, ntaux=1
	integer n0, n1, kbin, nt, family, ntb, nb, nintv, i, j, it, iopt, &
		iboot,parz, & 
		...
		npart, IIt(npart), &
		pm(nparm), pv(nparv), &
		II(2,nparr+npart+1), ir0(n0)

	double precision Z0(n0,nvarz), X0(n0), W0(n0), Z1(n1,nvarz), &
		X1(n1), W1(n1), &  
		hm(nparm), hv(nparv), t(nt), tb(ntb), Zb(nb,nvarz), &
		hroc(nparr+npart+1), &
		pvalue(2), &
		taux(ntaux)
...
!	******************************************************************
	...
	integer n0, n1, kbin, nb, nt, ntb, family, &
		nintv, i, j, it, iopt, &
		icont, nvarz, mode0z(nvarz), mode0(nvarz+1), &
		nparm, IIm(2,nparm), &
		nparv, IIv(2,nparv), &
		nparr, IIr(2,nparr), &
		npart, IIt(npart), &
	...
	double precision Z0(n0,nvarz), X0(n0), W0(n0), Z1(n1,nvarz), &
		...
		ROC(ntb,nb), AUC(nb), zmin, zmax, M10(n1), V10(n1), &
		Break(kbin,2,2),Cscoef(4,kbin,2,2),Err0(n0), &	
		pPV(n1), &
...
!	******************************************************************
	...
	logical optm, optacc(2), opttest, optaccROC, &
		optaccYI
	double precision X0(n0),X1(n1),Z0(n0),Z1(n1),W0(n0),W1(n1), &
	...
	allocate (Err0(n0),Err1(n1), &
		SH(nt),M0c(n1),V0c(n1), &
		M01(n1),V01(n1), &
		ROC_YI(ntyi,np),SH_YI(ntyi),AROCTH(ntyi), &
		tyi(ntyi), II0(2,npar),mode0(npar),II1(2,npar))
	...
	do i=1,np
		call cROC(M0p(i),M1p(i),V0p(i), V1p(i), Err0, Err1, W0, W1, &
			 n0, n1, t, nt, SH, ROC(1,i))
	...
	if(optacc(1).or.optacc(2)) then
		...
		if (optaccROC) then			
			do i=1,np
				call cROC(M0p(i),M1p(i),V0p(i), V1p(i), Err0, Err1, W0, W1,&
					n0, n1, tyi, ntyi, SH_YI, ROC_YI(1,i))		
				if(optaccYI) then
					maxt=1					
					YI(i)=0
					do j = 1, ntyi
	...
	deallocate (Err0,Err1,SH,M0c,V0c, & 
		M01,V01, &
		ROC_YI,SH_YI,AROCTH, &
		tyi,II0,II1,mode0)
	end
...
!	******************************************************************************
	...
	integer n(3),nb,nt,p,kbin, nboot,tagh, Status(n(1)), &
		i,j,it,k,boot,ir(n(1)),ir0(n(2)), ir1(n(3)),auxh, auxd, &
		ptemp(2,2), seed
	...
	!Confidence intervals
	if (cifit) then
		! Conditional on the true disease status
		Err0=(X0-M0)/sqrt(V0)
		Err1=(X1-M1)/sqrt(V1)
		do boot = 1, nboot
