	...
	if (n.cat > 0) {
		...
		calculate.set.cont <- function(cont, ind, card) {
			...
			if (all(!ind)) {
				...
				range <- c(min(data[ind,cont],na.rm = FALSE),max(data[ind,cont],na.rm = FALSE))
				if (range[1] == range[2]) {
		...
		for(i in 1:n.cont) {				
			range <- c(max(tapply(data[,names(cont)[i]],data[,group], min, na.rm = FALSE), na.rm = FALSE),min(tapply(data[,names(cont)[i]],data[,group],max,na.rm = FALSE),na.rm = FALSE))			 
